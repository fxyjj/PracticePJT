...
### 把 pp_name 加入到 project -> settings文件中的 INSTALL_APPS 中：
	`INSTALLED_APPS = [
...
### 在app_name目录下，新增一个urls.py文件，为接口新增路由：
...
### 把 app_name 下的urls添加到 project -> 下的 urls 中:
...
### django-cors-headers 来处理跨域:

	`pip install django-cors-headers`

### 在 project -> settings.py -> MIDDLEWARE 添加header参数：

	`MIDDLEWARE = [
		...
		'corsheaders.middleware.CorsMiddleware', <- 放在第三项，顺序注意
		...
	]
